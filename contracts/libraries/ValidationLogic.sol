...
library ValidationLogic {
	...
	*/
	function validateDeposit(DataTypes.ReserveData storage reserve, uint256 amount) external view {
		...
		require(reserve.avasTokenAddress != address(0), Errors.VL_INVALID_AMOUNT);
	...
	*/
	function validateWithdraw(
		...
		require(reserve.avasTokenAddress != address(0), Errors.VL_INVALID_AMOUNT);
		require(reserve.active, Errors.VL_NO_ACTIVE_RESERVE);
		...
		(uint256 sharerBalance, uint256 shareeBalance) = IToToken(reserve.token).balanceOfSharersPositionWithSharee(sharer, sharee);
